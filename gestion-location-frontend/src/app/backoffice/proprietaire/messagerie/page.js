...
export default function ProprietaireMessageriePage() {
  ...
  const [reclamationForm, setReclamationForm] = useState(EMPTY_RECLAMATION_FORM);
  ...
  async function handleSubmitReclamation(e) {
    e.preventDefault();
    if (!reclamationForm.sujet.trim() || !reclamationForm.message.trim()) return;
    setReclamationBusy(true);
    setReclamationBanner(null);
    try {
      const created = await createReclamation({
        sujet: reclamationForm.sujet.trim(),
